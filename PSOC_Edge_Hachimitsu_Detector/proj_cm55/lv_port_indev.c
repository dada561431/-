...
*
* Return:
*  cy_rslt_t: Initialization result from the selected touch driver
*
*******************************************************************************/
static cy_rslt_t touchpad_init(void)
{
...
void lv_port_indev_init(void)
{
    ...
    cy_rslt_t result = touchpad_init();

    if (CY_RSLT_SUCCESS != result)
    {
        printf("[GFX] Touch controller init failed with status = 0x%08lX; "
               "continuing without touch input.\r\n",
