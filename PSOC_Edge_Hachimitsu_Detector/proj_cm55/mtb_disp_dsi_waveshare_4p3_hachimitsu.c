...
static void mtb_disp_i2c_controller_recover(CySCB_Type* i2c_base,
                                                           ...
                                                           uint32_t buff_size)
{
    ...
    do
    {
        ...
        if (CY_SCB_I2C_SUCCESS == i2c_status)
        {
            ...
            if ((0U != timeout_count) &&
                (0U == (CY_SCB_I2C_MASTER_BUSY & i2c_controller_stat)) &&
                (!(I2C_CONTROLLER_ERROR_MASK & i2c_controller_stat)) &&
                (buff_size == Cy_SCB_I2C_MasterGetTransferCount(i2c_base,
                                                                i2c_context)))
            {
            ...
            else
            {
                ...
                mtb_disp_i2c_controller_recover(i2c_base, i2c_context);
                retry_count--;
            }
        }
        else
                                                   ...
                                                   cy_stc_scb_i2c_context_t* i2c_context)
{
    ...
    if (CY_SCB_I2C_SUCCESS == status)
    {
        ...
        if (CY_SCB_I2C_SUCCESS != status)
        {
            printf("[GFX] W4P3 ID read failed, status=0x%08lX\r\n",
        ...
        if (CY_SCB_I2C_SUCCESS == status)
        {
            ...
            {
                printf("[GFX] W4P3 unexpected display ID 0x%02X "
                       "(expected 0x%02X/0x%02X); continuing init.\r\n",
                       (unsigned int)device_id,
                       (unsigned int)MTB_DISP_WAVESHARE_4P3_ID_1,
                       (unsigned int)MTB_DISP_WAVESHARE_4P3_ID_2);
            }
            ...
            do
            {
                ...
                if (CY_SCB_I2C_SUCCESS != status)
                {
                    printf("[GFX] W4P3 init cmd[%u] 0x%02X=0x%02X failed, status=0x%08lX\r\n",
                           (unsigned int)index,
                           (unsigned int)mtb_disp_waveshare_4p3_init_cmds[index][CMD_POS],
                           (unsigned int)mtb_disp_waveshare_4p3_init_cmds[index][VAL_POS],
                           (unsigned long)status);
                }
